    ...
    <div class="col-lg-4">
        <div class="card h-100">
            ...
            <div class="card-body text-center">
                ...
                <div class="small text-muted">{{ $user->role?->name ?? 'No Role' }}</div>
                <div class="small text-muted">{{ $user->merchant?->business_name ?? 'Platform Account' }}</div>
                @if($avatarUrl)
                    <div class="mt-3">
                        <span class="badge bg-success">Profile photo uploaded</span>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="col-lg-8">
        <div class="card mb-4">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0">{{ __('messages.profile_settings') }}</h5>
                <span class="badge bg-light text-dark">{{ $user->isSuperAdmin() ? 'Super Admin' : ($user->merchant?->business_name ?? 'User') }}</span>
            </div>
                ...
                <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <small class="text-muted d-block mt-2">Upload a JPG, PNG, or WEBP image to replace the avatar shown in the navbar.</small>
                        </div>
                    </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6"><strong>Role:</strong> {{ $user->role?->name ?? 'N/A' }}</div>
                    <div class="col-md-6"><strong>Account Type:</strong> {{ $user->user_type ?? 'N/A' }}</div>
                    <div class="col-md-6"><strong>Merchant:</strong> {{ $user->merchant?->business_name ?? 'Platform' }}</div>
                    <div class="col-md-6"><strong>Last Login:</strong> {{ $user->last_login?->format('M d, Y H:i') ?? 'Never' }}</div>
                    <div class="col-md-6"><strong>Created:</strong> {{ $user->created_at?->format('M d, Y H:i') ?? 'N/A' }}</div>
                    <div class="col-md-6"><strong>Updated:</strong> {{ $user->updated_at?->format('M d, Y H:i') ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
