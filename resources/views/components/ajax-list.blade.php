...
(function (window, document) {
    ...
    function createSpinnerOverlay(container) {
        ...
        // make container position relative so overlay sits correctly
    ...
    window.initAjaxList = function initAjaxList(config) {
        // config: containerId (required), searchSelector, searchParam, formSelector, debounceMs
        ...
        async function doFetch(urlOrQ) {
            ...
            if (typeof urlOrQ === 'string' && (urlOrQ.startsWith('http') || urlOrQ.startsWith('/'))) {
                ...
                // if form exists, include all its inputs
                if (formEl) {
