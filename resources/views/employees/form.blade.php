...
@section('content')
<style>
    :root { --primary: #10b981; --primary-dark: #059669; --text-dark: #1f2937; --text-light: #6b7280; --border-light: #e5e7eb; --bg-light: #f9fafb; }
    * { transition: border-color 0.3s ease, box-shadow 0.3s ease, background-color 0.3s ease; }
    ...
    .form-control:focus, .form-select:focus { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(16, 185, 129, 0.12); outline: none; }
    ...
    .form-footer .btn-primary:hover { background: var(--primary-dark); transform: translateY(-2px); box-shadow: 0 8px 16px rgba(16, 185, 129, 0.3); }
                ...
                <form method="POST" action="{{ isset($employee) ? route('employees.update', $employee->id) : route('employees.store') }}">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $employee->name ?? '') }}" placeholder="Enter employee name" required>
                                    @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ __('messages.position') }} <span class="required">*</span></label>
                                    <input type="text" class="form-control @error('position') is-invalid @enderror" name="position" value="{{ old('position', $employee->position ?? '') }}" placeholder="Enter position" required>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('base_salary') is-invalid @enderror" name="base_salary" step="0.01" placeholder="0.00" value="{{ old('base_salary', $employee->base_salary ?? '') }}" required>
                    ...
                    <div class="form-footer">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle" style="margin-right: 8px;"></i>{{ isset($employee) ? __('messages.update') : __('messages.save') }}
                        </button>
        ...
        <div class="form-grid">
            ...
            <div class="form-sidebar">
                ...
                <div class="sidebar-card">
                    ...
                    <div class="sidebar-stat">
                        <span class="sidebar-stat-label">Joined</span>
                        <span class="sidebar-stat-value">{{ $employee->created_at?->format('M d, Y') ?? 'N/A' }}</span>
                    </div>
                    <div class="sidebar-stat">
                        <span class="sidebar-stat-label">Status</span>
                        <span class="sidebar-stat-value" style="color: {{ $employee->is_active ? '#10b981' : '#ef4444' }};">{{ $employee->is_active ? 'Active' : 'Inactive' }}</span>
            ...
            <div class="form-card">
                <div class="form-header">
                    <div>
                        <h1><i class="bi bi-person" style="margin-right: 10px;"></i>{{ isset($employee) ? __('messages.edit_employee') : __('messages.add_employee') }}</h1>
                    </div>
                ...
                <form method="POST" action="{{ isset($employee) ? route('employees.update', $employee->id) : route('employees.store') }}">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('base_salary') is-invalid @enderror" name="base_salary" step="0.01" value="{{ old('base_salary', $employee->base_salary ?? '') }}" required>
                                    @error('base_salary')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-footer">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save"></i> {{ isset($employee) ? __('messages.update') : __('messages.save') }}
                        </button>
                        <a href="{{ route('employees.index') }}" class="btn btn-secondary">
                            {{ __('messages.cancel') }}
                        </a>
